 ...
 * @Last Modified time: 2018-12-18 15:43:16
 * 错误聚合列表
 */
  ...
  <div>
    <el-card class="no-border" shadow="always">
      <el-table :data="tableData" style="width: 100%" v-loading="loading">
        <el-table-column prop="error_url" label="错误地址"></el-table-column>
...
export default {
  ...
  methods: {
    async getList() {
      ...
      const { code, data } = await this.$monitoringError.aggregateErrorList(
        params
      );
      if (code == 0) {
        this.loading = false;
